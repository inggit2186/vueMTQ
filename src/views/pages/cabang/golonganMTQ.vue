...
export default {
    ...
    computed: {
		...
		ruteTujuan() {
			return (id,teamstatus,team) => `/cabangmtq/${this.navid}/${teamstatus}/${team}/${this.cid}/${id}`
		}
	},
	created() {
		this.getCabang(),
	...
	methods: {
		async getCabang() {
			...
			try{
				...
				const response = await this.$axios.get(import.meta.env.VITE_APP_API_URL+'/cabangMTQ/'+this.navid+'/'+sid,{headers})
